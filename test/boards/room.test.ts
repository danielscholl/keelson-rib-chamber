...
// The folded tool group row a turn's calls render as (icon "⚙"), in feed order.
...
describe("buildRoomBoard · observability", () => {
  ...
  test("a turn's trailing carries token spend but NOT the tool count (tools fold into one row)", () => {
    const board = buildRoomBoard(room(), [
      entry({
        turnIndex: 0,
    ...
    // The turn's two tools fold into ONE collapsed group row.
    expect(toolRowsIn(board)).toHaveLength(1);
    expect(toolRowsIn(board)[0]?.text).toBe("2 tools");
  });
  ...
  test("a turn's tools fold into one group row — count on the face, per-tool list + inputs under the caret", () => {
    const board = buildRoomBoard(room(), [
      entry({
        toolCalls: [
          { name: "view", input: '{\n  "path": "README.md"\n}' },
    ...
    // ONE collapsed row for the whole group; its face is just the count.
    expect(rows).toHaveLength(1);
    const group = rows[0];
    expect(group?.text).toBe("3 tools");
    // The per-tool list — name · FAMILY (+ · failed), then the input JSON — lives in detail.
    const detail = group?.detail ?? "";
    expect(detail).toContain("view · BUILT-IN");
    expect(detail).toContain('"path": "README.md"');
    expect(detail).toContain("chamber_table_exhibit · CHAMBER");
    expect(detail).toContain("Bash · BUILT-IN · failed");
    // A persisted family (from the raw MCP name) labels the line, not the stripped name.
    const mcp = buildRoomBoard(room(), [
      entry({ toolCalls: [{ name: "shell", family: "mcp", input: '{ "cmd": "ls" }' }] }),
    ]);
    expect(toolRowsIn(mcp)[0]?.detail).toContain("shell · MCP");
    // A no-input call still lists its head line (no JSON body).
    const noInput = buildRoomBoard(room(), [entry({ toolCalls: [{ name: "Grep" }] })]);
    expect(toolRowsIn(noInput)[0]?.detail).toBe("Grep · BUILT-IN");
    // Success is NOT asserted (absent errored ≠ confirmed ok); only known failures are
    // surfaced on the group row — the count of them, in the error tone.
    expect(group?.trailing).toBe("1 failed");
    expect(group?.glyph).toBe("error");
    // A group with no failures wears no error tone or trailing.
    const clean = buildRoomBoard(room(), [entry({ toolCalls: [{ name: "view" }] })]);
    expect(toolRowsIn(clean)[0]?.trailing).toBeUndefined();
  ...
  test("the group detail lists bare tool names (no category-word marker doubling the verb)", () => {
    // "view" carries a "read" kind marker in toolPresentation; the line must not read "read view".
    const board = buildRoomBoard(room(), [
      entry({
        toolCalls: [{ name: "view", input: '{ "path": "README.md" }' }, { name: "foo__bar" }],
      }),
    ]);
    const detail = toolRowsIn(board)[0]?.detail ?? "";
    expect(detail).toContain("view · BUILT-IN");
    // foo__bar's family is inferred as "foo" (the segment before the first "_").
    expect(detail).toContain("foo__bar · FOO");
