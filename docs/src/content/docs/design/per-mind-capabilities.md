...
tools, the genesis write seam, or another rib's tools, even with a hand-edited
`mind.json`. The two layers are belt-and-suspenders: the core seam enforces the
operator floor, and the rib enforces least privilege per room.
