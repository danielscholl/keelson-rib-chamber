...
to disk and the panel reflects it, so you see the workspace go from a few preset
...
The easiest way to open one is the convene composer, folded into the Chamber panel
at the top of the surface. Click a Mind's seat card to bring them to the table, so
with two Minds authored you seat both. Add a topic so the room has something to chew
on, then
...
Once the room starts, its card appears on the Rooms index. Click **Open** on that
card and the room's board streams into a drawer, turns landing round by round. You do
not drive them. The room advances on its own, running one Mind, then the next, then
back to the first, until it reaches its budget.
...
transcript is the whole conversation, and why the room's board is just that
...
When the room reaches its budget it ends on its own. The board holds the finished
transcript, oldest turn at the top, each entry labeled with the Mind that spoke it.
Read it top to bottom and you can follow the whole exchange: the first
...
A finished room stays as bounded history. **Open** on its card rebuilds the
transcript from disk and shows it in the drawer, so you can read the exchange again
whenever you like.

## Stop the room

A room you started will run to its budget, but you do not have to wait. To end it
early, **Open** the room and use the stop control on its board, or the
`chamber_room_stop` chat tool. Stopping is reversible: it ends the current room, and you can start a fresh
