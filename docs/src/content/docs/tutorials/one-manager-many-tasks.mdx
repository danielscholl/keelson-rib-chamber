...
- **Or let a Mind build it, in a coding room.** A coding room (started with
  `coding: true`) lifts the text-only default: a Mind that declares the `read` and
  `code` capabilities can read files and run Bash, Edit, and Write, confined to a
  project you target when the room starts. Hand a build Mind the manager's plan and
