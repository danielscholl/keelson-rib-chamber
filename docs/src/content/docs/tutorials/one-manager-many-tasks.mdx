...
orchestration and reasoning). Pin from a card's model dropdown, or pre-pin
at authoring time:
...
- **Or let a Mind build it, in a coding room.** A coding room (started with
  ...
  project you target when the room starts. Author a build engineer, pin it to a
  gpt-class model (direct coding is its strength), give it `read` and `code`, and
  convene a coding room pointed at your project. Hand it the manager's plan and ask it to implement the app. Because the
