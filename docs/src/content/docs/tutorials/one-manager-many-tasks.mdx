...
orchestration and reasoning). Pin from a card's model picker, or pre-pin
at authoring time:
...
Two tasks, two owners, two states. The manager wrote it; the driver settles a
worker's row to `completed` when its turn lands cleanly (or `failed` if the turn
aborts or errors), so the row reflects the turn's outcome, not a worker's
self-report. `status: "executing"` means open tasks remain,
...
- **Or let a Mind build it, in a coding room.** A coding room (started with
  ...
  capability can run Bash, Edit, and Write from a project you target when the room
  starts (the file tools are confined to it; a command Bash runs is not). Reading that project needs no declaration at all, because
  targeting it grants every speaker read-only access to its root. Hand a build Mind the manager's plan and
  it implements inside the room's confinement boundary, reading whatever build
  guidance the project ships along the way. That is the next tutorial,
  [A room that writes code](../a-room-that-writes-code/), which walks the whole
  session: authoring the builder, opening the tier deliberately, and verifying
...
moderated discussion, an unmoderated floor, and a manager-led build. Two
tutorials remain, and they hand off to each other. First,
[A room that writes code](../a-room-that-writes-code/) opens the coding tier
and lets a build Mind implement a scoped change while a reviewer reads behind
it. Then the capstone,
[author your own room strategy](../author-a-room-strategy/), has you write the
pure routing policy the driver consults every turn, the same seam all six
shipped strategies plug into.

## Related

- [A room that writes code](../a-room-that-writes-code/): the next step, a coding room implements what the manager planned.
