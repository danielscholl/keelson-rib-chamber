...
> **Update (2026-07-14): superseded — rooms hold no panel.** The `retainOnly` retention mechanism named below is gone. `room-region-registry.ts` is now `room-key-registry.ts` (`createRoomRegionRegistry` → `createRoomKeyRegistry`): it registers a per-slug snapshot key and exposes `release(slug)`, with no surface region — so there is no retention policy to run and no harness region ceiling to stay under. A room is an activity entered from the Rooms index (`Open` focuses the driver's per-slug key), not a standing panel, so `MAX_ACTIVE_ROOMS` (now in `src/room-config.ts`) no longer has a region ceiling to stay under — it remains an admission cap on `room-start`, bounding the concurrent paid-turn fan-out on cost alone. The banners and text below are retained for history.
