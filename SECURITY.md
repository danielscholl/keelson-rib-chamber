...
Thanks for taking the time to make the Chamber rib safer.

## Supported versions

The Chamber rib is pre-1.0 (`0.x`) software. Security fixes land on the
...
- Email: **degnome@gmail.com** with subject line `[chamber security]`
- GitHub private vulnerability report:
  <https://github.com/danielscholl/keelson-rib-chamber/security/advisories/new>

A useful report includes:

- A description of the issue and the impact you observed (or believe is
  possible)
- The Chamber rib version, your Keelson version (`keelson version --json`),
...
Chamber is a **Keelson rib** — a capability package installed into the
Keelson harness and discovered at boot. It coordinates multiple agent
Minds across shared rooms and calls provider APIs. A rib runs with the
same privileges as the harness, so the threat model assumes:
...
- Hostile inputs may arrive over the network from provider responses,
  Mind turn outputs, tool outputs, or fetched URLs.

### In scope

- Command injection or path traversal in any rib handler that splices
  untrusted data (provider output, Mind/room transcript content, upstream
  node output, workflow YAML fields) into a shell, child process, or
  filesystem path
- Credential or secret leakage: any path where the rib logs, snapshots,
  or transmits provider tokens or other secrets it handles
